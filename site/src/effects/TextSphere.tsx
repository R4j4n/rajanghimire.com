/**
 * Word Globe — ported from references/reference4.html (Originkit).
 *
 * A rotating sphere whose surface is a phrase repeated along weighted
 * longitude bands. Glyphs are laid out by walking each band in arc-length
 * steps sized by the measured character widths, then projected with a
 * fixed camera distance so the silhouette stays circular.
 */
...
const buildGeometry = (
    ...
    const characterWidths = characters.map((c) => context.measureText(c).width)
...
export default function TextSphere({
    ...
    }, [word, color, fontFamily, fontSize, fontWeight, speed, rotationSide, twist, letterSpacing])
    ...
    useEffect(() => {
        ...
        const renderFrame = (timestamp: number) => {
            ...
            if (disposed || !isVisible || canvasWidth === 0 || canvasHeight === 0) return
            ...
            projected.forEach((glyph) => {
                const rawGlyphSize = p.fontSize * glyph.scale * logicalScale
                ...
                const nextFont = `${p.fontWeight} ${size.toFixed(1)}px ${p.fontFamily}`
