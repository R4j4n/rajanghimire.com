/**
 * Reactive Lines — ported from references/reference5.html (Originkit).
 *
 * A field of quadratic-ish curves fanning from one corner; the cursor's
 * vertical position drives how many lines are drawn and its horizontal
 * position skews the displacement bias. Used as the hero backdrop.
 */
    ...
    const { containerRef, canvasRef, stateRef } = useCanvasAnimation({
        ...
        onDraw: (e, t) => {
            ...
            for (let i = 0; i < o.linesNum; i++) {
                ...
                const lineEnd = vec(
                    lerp(f.x, g.x, 1 - ratio * ratio),
                    lerp(f.y, g.y, 1 - ratio * ratio)
                )

                const l = vecAdd(vecMult(c, 0.5), vecMult(lineEnd, 0.5))
                const dispTarget = vecMult(vecAdd(f, l), 0.5)

                // Draw one displaced curve between `c` and `lineEnd`.
                const mid = vecLerp(c, lineEnd, 0.5)
                const off = vecSub(dispTarget, mid)

                e.beginPath()
                for (let k = 0; k <= 50; k++) {
                    const p = k / 50
                    const base = vecLerp(c, lineEnd, p)
                    const w =
                        2 *
                        Math.pow(p, d * (1 - o.bias) * 2) *
                        Math.pow(1 - p, d * o.bias * 2)
                    const cv = vecAdd(base, vecMult(off, w))
                    k === 0 ? e.moveTo(cv.x, cv.y) : e.lineTo(cv.x, cv.y)
                }
                e.stroke()
            }
            ...
            if (fade) {
                ...
                e.translate(y, v)
                e.scale(r / (2 * x), n / (2 * x))
                const grd = e.createRadialGradient(0, 0, 0, 0, 0, x)
                grd.addColorStop(0, rgba(0))
                grd.addColorStop(inner, rgba(0))
                grd.addColorStop(lerp(inner, 1, 0.5), rgba(maxA * 0.3))
                grd.addColorStop(lerp(inner, 1, 0.8), rgba(maxA * 0.7))
                grd.addColorStop(1, rgba(maxA))
                e.fillStyle = grd
